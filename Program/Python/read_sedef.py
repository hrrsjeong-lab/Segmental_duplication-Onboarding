...
if __name__ == "__main__":
    ...
    parser.add_argument("input", help="Input BED file", type=str)
    ...
    step00.check_suffix(args.input, {".bed"})
    step00.check_suffix(args.reference, {".bed", ".bed.gz"})
    step00.check_suffix(args.length, {".txt"})
    step00.check_suffix(args.output, {".zip"})

    column_data_type_dict = {"Chromosome 1": str, "Start 1": int, "End 1": int, "Chromosome 2": str, "Start 2": int, "End 2": int, "Name": str, "Score": float, "Strand 1": str, "Strand 2": str, "Maximum length": int, "Aligned length": int, "cigar": str, "comment": str, "indel_a": int, "indel_b": int, "Aligned bases": int, "Match bases": int, "Mismatch bases": int, "Transition bases": int, "Transversions": int, "Match fraction": float, "Match indel fraction": float}
    type_palette = {"SEDEF": "tab:green", "Reference": "tab:orange"}
    type_marker = {"SEDEF": "P", "Reference": "X"}
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        # ECDF plots
        for x in tqdm.tqdm(["Match fraction", "Match indel fraction"], leave=False):
        ...
        # Hist plots
        for x in tqdm.tqdm(["Aligned length", "Maximum length", "Aligned bases"], leave=False):
            fig, ax = matplotlib.pyplot.subplots(figsize=(18, 18))
        ...
        matplotlib.pyplot.bar(range(len(chromosome_list)), length_data["SEDEF"], width=-0.4, align="edge", color=type_palette["SEDEF"], linewidth=0, label="SEDEF")
        ...
        for i, chromosome in tqdm.tqdm(enumerate(chromosome_list), total=len(chromosome_list)):
            input_length = length_data.loc[chromosome, "SEDEF"]
        ...
        for chromosome in tqdm.tqdm(chromosome_list):
            ...
            for index, row in tqdm.tqdm(selected_input_data.iterrows(), total=len(selected_input_data), position=1, leave=False):
                matplotlib.pyplot.barh(1.0, (row["End 1"] - row["Start 1"]), left=row["Start 1"], color=type_palette["SEDEF"], linewidth=0)

            for index, row in tqdm.tqdm(selected_reference_data.iterrows(), total=len(selected_reference_data), position=1, leave=False):
                matplotlib.pyplot.barh(-1.0, (row["End 1"] - row["Start 1"]), left=row["Start 1"], color=type_palette["Reference"], linewidth=0)

            matplotlib.pyplot.text(length_data.loc[chromosome, "Length",] / 2, 1.0, f"{overlap_length / length_data.loc[chromosome, 'SEDEF'] * 100:.2f}%", color="black", fontsize="large", horizontalalignment="center", verticalalignment="center", path_effects=step00.path_effects)
            ...
            matplotlib.pyplot.yticks([-1.0, 0.0, 1.0], ["Reference", "Overlap", "SEDEF"])
            matplotlib.pyplot.grid(True)
            matplotlib.pyplot.tight_layout()
        ...
        for query_chromosome, target_chromosome in tqdm.tqdm(sorted(chromosome_pair_set)):
            drawing_data = pandas.concat([input_data.loc[(input_data["Chromosome 1"] == query_chromosome) & (input_data["Chromosome 2"] == target_chromosome)], reference_data.loc[(reference_data["Chromosome 1"] == query_chromosome) & (reference_data["Chromosome 2"] == target_chromosome)]], ignore_index=True, join="inner")
            drawing_data = drawing_data.loc[(drawing_data["Match fraction"] >= 0.9)]

            # Scatter
            fig, ax = matplotlib.pyplot.subplots(figsize=(18, 18))

            seaborn.scatterplot(data=drawing_data, x="Start 1", y="Start 2", hue="Match fraction", palette="YlOrRd", style="Type", markers=type_marker, legend="brief", s=100, edgecolor=None, rasterized=True, ax=ax)
